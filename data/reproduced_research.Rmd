...
Here follows the actual EDA. It is written entirely using the _tidyverse_ packages as well as some others. Study it in pairs or trios and report flaws, errors, weaknesses and possible solutions/repairs at the end of the session.
...
```Check one last time for any grammar/ spelling mistakes.```

```"There should be a reasoning added as to why these particular libraries are used"```
...
```A positive note is that the author added labels to each function, which makes it easier to detect a bug in a code chunk! :)```
...
```"There is no reasoning given for this code block. If you remove data, there should always be a reason why."```
...
```"There should be text added to the illustration"```
...
```"We do not know where the numeric colums are refactored to factors"```
...
```"Declare which values are used for the boxplot, and also describe why those are needed"```
...
```"The value 210 might have a reason behind it. Maybe you needed to filter the data better? because garbadge in = garbadge out"```
...
```Provide the link in a proper manor instead of just dumping it right here, and credit the author. This code is also very hard to read, one problem might be the variable names which are very similar. If I would have needed to editted the code, I would be unable because of inreadability"```
...
```This part cannot be redone, no one would understand```
...
```Give reasoning as to why it might be maxhr. Also again, code without comments, which makes it less readable```
...
```"What are the boxplots illustrating? and what are the conclusions?"```
